...
// Styled select to match TireFinder
const StyledSelect = ({ value, onChange, options, placeholder, disabled, style }) => (
  <select
    value={value}
    onChange={(e) => onChange(e.target.value)}
    disabled={disabled}
...
// Styled input to match TireFinder
const StyledInput = ({ value, onChange, placeholder, type = 'text', style, ...props }) => (
  <input
    type={type}
    value={value}
...
export default function QuoteBuilder() {
  ...
  const [selectedStore, setSelectedStore] = useState(() => {
    if (typeof window !== 'undefined') return localStorage.getItem('jl_tire_store') || '609';
    return '609';
  });
  ...
  // 4 tread depth inputs: Front Left, Front Right, Rear Left, Rear Right
  const [treadFL, setTreadFL] = useState('');
  const [treadFR, setTreadFR] = useState('');
  const [treadRL, setTreadRL] = useState('');
  const [treadRR, setTreadRR] = useState('');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('jl_tire_store', selectedStore);
  }, [selectedStore]);
  ...
  const getLowestTread = () => {
    const depths = [treadFL, treadFR, treadRL, treadRR].filter(d => d !== '' && !isNaN(parseInt(d))).map(d => parseInt(d));
    return depths.length > 0 ? Math.min(...depths) : null;
  };
    ...
    try {
      const requestBody = {
        ...
        customer: {
          ...
          license_plate: licensePlate || null, license_state: licensePlate ? licenseState : null,
          data_source: customerData.data_source
        },
        ...
        tread_depth: lowestTread !== null ? {
          front_left: treadFL ? parseInt(treadFL) : null, front_right: treadFR ? parseInt(treadFR) : null,
          rear_left: treadRL ? parseInt(treadRL) : null, rear_right: treadRR ? parseInt(treadRR) : null
        } : null,
        tire: {
  ...
  if (!tireData) {
    return (
      <div style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: '100vh', background: 'linear-gradient(180deg, #e8f4f8 0%, #d0e8f0 100%)' }}>
        <Header selectedStore={selectedStore} setSelectedStore={setSelectedStore} />
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 150px)', padding: '20px' }}>
        ...
        <div style={{ maxWidth: '600px', margin: '30px auto', padding: '0 20px' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '15px', padding: '40px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', textAlign: 'center' }}>
            ...
            <div style={{ backgroundColor: '#f8f9fa', borderRadius: '10px', padding: '20px', marginBottom: '25px', textAlign: 'left' }}>
              <p style={{ margin: '0 0 8px 0' }}><strong>Customer:</strong> {generatedQuote.customer?.name || 'Not provided'}</p>
              {generatedQuote.vehicle && <p style={{ margin: '0 0 8px 0' }}><strong>Vehicle:</strong> {generatedQuote.vehicle}</p>}
              <p style={{ margin: '0 0 8px 0' }}><strong>Tire:</strong> {generatedQuote.tire?.brand} {generatedQuote.tire?.name}</p>
            ...
            <div style={{ display: 'flex', gap: '15px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={handlePrint} style={{ backgroundColor: '#9b59b6', color: 'white', border: 'none', padding: '12px 25px', borderRadius: '25px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>🖨️ Print Quote</button>
              <button onClick={handleEmailQuote} style={{ backgroundColor: '#27ae60', color: 'white', border: 'none', padding: '12px 25px', borderRadius: '25px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>📧 Email Quote</button>
            </div>
            <div style={{ marginTop: '25px', paddingTop: '20px', borderTop: '1px solid #eee' }}>
    ...
    <div style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: '100vh', background: 'linear-gradient(180deg, #e8f4f8 0%, #d0e8f0 100%)' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '30px 20px' }}>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '15px', padding: '40px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
          ...
          <h2 style={{ color: '#9b59b6', fontSize: '28px', fontWeight: '700', textAlign: 'center', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '3px' }}>
            CREATE QUOTE
          </h2>
          <p style={{ color: '#888', textAlign: 'center', fontSize: '13px', marginBottom: '30px' }}>
            TIRE QUOTE BUILDER
          </p>

          {/* Selected Tire Summary */}
          <div style={{ backgroundColor: '#9b59b6', borderRadius: '10px', padding: '20px', color: 'white', marginBottom: '30px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
              <div>
                <h3 style={{ margin: '0 0 5px 0', fontSize: '18px' }}>{tireData.brand_code || tireData.brand} {tireData.tire_size || tireData.size} {tireData.name || tireData.sales_class}</h3>
          ...
          {/* Vehicle Info */}
          {vehicleData?.display && (
            <div style={{ backgroundColor: '#f0f8ff', border: '2px solid #9b59b6', borderRadius: '10px', padding: '15px 20px', marginBottom: '25px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontSize: '24px' }}>🚗</span>
              <span style={{ fontWeight: '600', color: '#333', fontSize: '15px' }}>{vehicleData.display}</span>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: '1', minWidth: '300px' }}>
              ...
              <div style={{ marginBottom: '25px' }}>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
                  <div style={{ flex: 1, height: '1px', backgroundColor: '#9b59b6', marginRight: '8px' }}>
                    <div style={{ width: '7px', height: '7px', backgroundColor: '#9b59b6', borderRadius: '50%', marginTop: '-3px' }} />
                  </div>
              ...
              <div style={{ marginBottom: '25px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                  <StyledInput value={licensePlate} onChange={setLicensePlate} placeholder="LICENSE PLATE" style={{ flex: 1 }} onKeyPress={(e) => e.key === 'Enter' && handleCustomerLookup()} />
                  <StyledSelect value={licenseState} onChange={setLicenseState} options={US_STATES} placeholder="ST" style={{ width: '80px' }} />
                  <button onClick={handleCustomerLookup} disabled={!licensePlate.trim() || customerLookupLoading} style={{ backgroundColor: '#9b59b6', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '25px', fontSize: '12px', fontWeight: '600', cursor: licensePlate.trim() ? 'pointer' : 'not-allowed', opacity: licensePlate.trim() ? 1 : 0.5 }}>
                    {customerLookupLoading ? '...' : '🔍'}
                  </button>
                </div>

                {customerFound && (
                  <div style={{ backgroundColor: '#f0fff4', border: '1px solid #27ae60', borderRadius: '8px', padding: '10px 15px', marginBottom: '15px', fontSize: '12px', color: '#27ae60' }}>
                    ✓ Customer found! Info loaded below.
                  </div>
                )}

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <StyledInput value={customerData.first_name} onChange={(v) => setCustomerData({...customerData, first_name: v})} placeholder="FIRST NAME" style={{ width: '100%' }} />
                  <StyledInput value={customerData.last_name} onChange={(v) => setCustomerData({...customerData, last_name: v})} placeholder="LAST NAME" style={{ width: '100%' }} />
                  <StyledInput value={customerData.phone} onChange={(v) => setCustomerData({...customerData, phone: v})} placeholder="PHONE" style={{ width: '100%' }} />
                  <StyledInput value={customerData.email} onChange={(v) => setCustomerData({...customerData, email: v})} placeholder="EMAIL" style={{ width: '100%' }} />
                </div>
              </div>
            </div>

            {/* Right Column: Tread Depth & Options */}
            <div style={{ flex: '1', minWidth: '300px' }}>
              
              {/* Tread Depth Section - 4 inputs */}
              <div style={{ marginBottom: '25px' }}>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
                  <span style={{ color: '#9b59b6', fontSize: '11px', fontWeight: '700', letterSpacing: '1px' }}>CURRENT TREAD DEPTH</span>
                ...
                {/* Car diagram with 4 tread inputs */}
                <div style={{ position: 'relative', padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    {/* Left side - Front/Rear Left */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                      <TreadInput label="FL" value={treadFL} onChange={setTreadFL} />
                      <TreadInput label="RL" value={treadRL} onChange={setTreadRL} />
                    </div>
                    ...
                    {/* Right side - Front/Rear Right */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                      <TreadInput label="FR" value={treadFR} onChange={setTreadFR} />
                      <TreadInput label="RR" value={treadRR} onChange={setTreadRR} />
                    </div>
                  </div>

                  {/* Tread Status */}
                  {lowestTread !== null && (
                    <div style={{ marginTop: '15px', padding: '10px 15px', backgroundColor: treadStatus?.color + '20', border: `2px solid ${treadStatus?.color}`, borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <span style={{ fontSize: '18px', fontWeight: '700', color: treadStatus?.color }}>{lowestTread}/32"</span>
                        <span style={{ marginLeft: '10px', fontSize: '11px', fontWeight: '700', color: treadStatus?.color }}>{treadStatus?.label}</span>
                      </div>
                      <div style={{ textAlign: 'right', fontSize: '10px', color: '#666' }}>
                        <div>Current: ~{stoppingDistance}ft</div>
                        <div>New: ~220ft</div>
                      </div>
                    </div>
                  )}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  <div>
                    <label style={{ fontSize: '10px', color: '#888', display: 'block', marginBottom: '5px', textAlign: 'center' }}>QTY</label>
                    <StyledSelect value={quantity} onChange={(v) => setQuantity(parseInt(v))} options={QTY_OPTIONS} placeholder="QTY" style={{ width: '70px' }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: '10px', color: '#888', display: 'block', marginBottom: '5px', textAlign: 'center' }}>REBATE $</label>
                    <StyledInput type="number" value={rebateAmount} onChange={setRebateAmount} placeholder="0.00" style={{ width: '100%' }} />
                  </div>
                  <div style={{ flex: 2 }}>
                    <label style={{ fontSize: '10px', color: '#888', display: 'block', marginBottom: '5px', textAlign: 'center' }}>REBATE DESC</label>
                    <StyledInput value={rebateDescription} onChange={setRebateDescription} placeholder="e.g., Nexen Spring Rebate" style={{ width: '100%' }} />
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Error */}
          {error && (
            <div style={{ backgroundColor: '#fee', border: '1px solid #e74c3c', borderRadius: '8px', padding: '12px 15px', marginTop: '20px', color: '#c0392b', fontSize: '13px', textAlign: 'center' }}>
              {error}
            </div>
          )}
...
// Header Component matching TireFinder
const Header = ({ selectedStore, setSelectedStore }) => (
  <>
    <header style={{ backgroundColor: 'white', padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
      <img src="https://www.jiffylube.com/-/media/images/jiffylube/logos/jl-multicare-logo-color.png" alt="Jiffy Lube Multicare" style={{ height: '50px' }} onError={(e) => { e.target.onerror = null; e.target.src = 'https://www.jiffylube.com/-/media/images/jiffylube/logos/jl-logo-white.png'; }} />
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ color: '#333', fontSize: '13px', fontWeight: '600' }}>STORE:</span>
        <select value={selectedStore} onChange={(e) => setSelectedStore(e.target.value)} style={{ padding: '8px 15px', border: '2px solid #9b59b6', borderRadius: '20px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', outline: 'none' }}>
          {STORES.map(store => (<option key={store.id} value={store.id}>{store.id} - {store.name}</option>))}
        </select>
      </div>
    </header>
    <nav style={{ backgroundColor: '#9b59b6', padding: '12px 20px', display: 'flex', justifyContent: 'center', gap: '40px' }}>
      <a href="#/" style={{ color: 'white', textDecoration: 'none', fontSize: '12px', fontWeight: '700', letterSpacing: '2px', padding: '5px 15px', borderRadius: '20px', backgroundColor: 'transparent' }}>TIRE FINDER</a>
      <a href="#/inventory" style={{ color: 'white', textDecoration: 'none', fontSize: '12px', fontWeight: '700', letterSpacing: '2px', padding: '5px 15px', borderRadius: '20px', backgroundColor: 'transparent' }}>STORE INVENTORY</a>
    </nav>
  </>
);

// Tread Input Component
const TreadInput = ({ label, value, onChange }) => (
  <div style={{ textAlign: 'center' }}>
    <label style={{ fontSize: '10px', color: '#888', fontWeight: '600', display: 'block', marginBottom: '3px' }}>{label}</label>
    <input
      type="number"
      min="0"
      max="12"
